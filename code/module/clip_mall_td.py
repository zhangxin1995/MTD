...
# 1/0
def sinusoidal_position_embedding(t, output_dim, device):
    position = t
...
def RoPE(q,t,c,output_dim=None):

    if output_dim is None:
        output_dim = q.shape[-1]

    # # (bs, head, max_len, output_dim)
    # pos_emb = sinusoidal_position_embedding(t, output_dim, q.device)
...
def umapf(name,ori_fs,feats,real_paths=None,predict_paths=None):
    '''
    使用umap方法可视化
    Args:
        writer (object): 日志实例
    ...
    Returns:
        None
    '''
    reducer = UMAP(n_neighbors=14,random_state=256,metric='cosine',n_components=2,n_epochs=1000,learning_rate=0.5)
    plt.figure(figsize=(12, 6))
    X_embedded = reducer.fit_transform(ori_fs.cpu().detach().numpy())
    y0=list(range(feats.size(0)))
    for i,p  in enumerate(real_paths):
        for k in p:
            y0[k]=i
        # plt.plot(X_embedded[p,0],X_embedded[p,1])
    plt.scatter(X_embedded[:,0], X_embedded[:,1],c=y0,cmap='rainbow',s=45)   
    plt.savefig('results/{}_scatter.eps'.format(name),dpi=300) 
    
    X_embedded = reducer.fit_transform(feats.cpu().detach().numpy())
    plt.figure(figsize=(12, 6))
    if real_paths is None:
        plt.scatter(X_embedded[:,0], X_embedded[:,1],cmap='Reds',s=45)# 根据y值进行可视化 选择颜色值
    else:
        y0=list(range(feats.size(0)))
        for i,p  in enumerate(real_paths):
            for k in p:
    ...
    plt.savefig('results/{}_real.eps'.format(name),dpi=300) 
    ...
    if predict_paths is not None and real_paths is not None:
        ...
        plt.scatter(X_embedded[:,0], X_embedded[:,1],c=y0,cmap='rainbow',s=45)    
        # buffer = BytesIO()
        plt.savefig('results/{}_predict.eps'.format(name),dpi=300)
        # img_array = np.asarray(Image.open(buffer))
        # # print(dir(writer.add_image))
...
class TrackletEmbedding(nn.Module):
    ...
    def draw_distance(self,st,eta):
        a=torch.ones((1,int(self.dim)*2))
        a1=self.RoPE(a,0,0)
        xs=[]
        ys=[]
        for i in tqdm(range(86400)):
            t=process_ts(i*1000+st,eta=eta)
            a2=self.RoPE(a,t,0)
            xs.append(i)
            ys.append(torch.sum(a1*a2).detach().numpy())
            # break
        plt.plot(xs,ys)
...
def fcluster_adj2path_from_dist(dist,threshold=0.2,criterion='distance'):
    ...
    if len(dist.shape)==2:
        ...
        new_dist=dist
    new_dist[np.where(new_dist<0)[0]]=0
    print(new_dist[np.where(new_dist<0)[0]])
...
def extract_feat_ts_dist(model,feat,ts,cs,blocking=True):
    ...
    dist=build_evaluate(encode_feats, encode_feats, 'cosine',need_norm=True) 
    if blocking:
...
def clip_mall_td_main(config):
    ...
    for ss in range(100):
        ...
        seed=config['seed']
        config['seed']
        embedding_model=TrackletEmbedding(dim=qfs.shape[1]/2,seed=seed).cuda()
        trajectory_extract_model=VehicleRouteResolution()
        # embedding_model.draw_distance(1634400000000,config['manifold']['eta'])
        ...
        values_error=[]
        values_t_error=[]
        for ab,key in enumerate(candidate_set.keys()):
        ...
        for ab,key in enumerate(candidate_set.keys()):
            ...
            # print(ts)
            # input()
            arg_ts=np.argsort(ts.cpu().numpy())
            ts=[ts[k] for k in arg_ts]
            path=[path[k] for k in arg_ts]
            ...
            sub_pids2idxs=defaultdict(list)
            # if len(path)>5:
            #     real_paths=[]
            #     for i,p in enumerate(path):
            #         idx=int(gpids[p])
            #         sub_pids2idxs[idx].append(i)
            #     for key in sub_pids2idxs.keys():
            #         real_paths.append(sub_pids2idxs[key])
                
            #     predict_paths=paths
            #     # print(real_paths)
            #     # print(predict_paths)
            
            #     umapf(ab,features,encode_feats,real_paths=real_paths,predict_paths=predict_paths)
            #     # input()

    
        print(len(candidate_set_path))
        # exit()
