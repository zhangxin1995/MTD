...
def process_ts(ts,eta=1):
    new_ts=(ts-1634373468160)/100/86400*eta
    return new_ts
    ...
    def trajectory_generation(self,dist,trajectory_num,weight=-1,weights=None,para=1):
        ...
        for i in range(1,r):
            ...
            if d<min_d:
                ...
                min_d=d
        num=[]
        for p in  sub_paths[min_d_idx]:
            num+=p
        num=len(set(num))
        assert num==dist.shape[0],'{} {}'.format(num,dist.shape[0])
...
def manifold_mall_td_main(config):
    for ss in range(100):
        epoch=0
        ...
        embedding_model=TrackletEmbedding(dim=qfs.shape[1]/2,seed=seed)
        ...
        for ab,key in enumerate(candidate_set.keys()):
            ...
            # print(path)
            # print(gts[path],gpids[path])
            ts=process_ts(gts[path].cpu(),eta=config['manifold']['eta'])
            arg_ts=np.argsort(ts)
        ...
        for path in paths:
            ...
            num+=len(gls[-1])
        assert num==gpids.size(0),'{} {}'.format(num,gpids.size())
